...
void main() {
  ...
  test('downloads and extracts one curve per indexed profile', () async {
    final tmp = Directory.systemTemp.createTempSync('flax_autoeq_');
    addTearDown(() => tmp.deleteSync(recursive: true));

    final db = AutoEqDatabase(cacheDirOverride: tmp);
    expect(await db.isAvailable, isFalse, reason: 'starts with an empty cache');
    ...
    expect(await db.isAvailable, isTrue,
    ...
    expect(curves, greaterThan(index.length - 10),
