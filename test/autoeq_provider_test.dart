...
void main() {
  ...
  test('a completed download leaves the profile list populated', () async {
    ...
    expect(
      notifier.state.searchResults,
      isNotEmpty,
      reason: 'results must be loaded once the database becomes available',
    );
    expect(
      notifier.state.searchResults.map((p) => p.name),
  ...
  test('a profile saved before the download resolves afterwards', () async {
    ...
    expect(
      notifier.state.activeProfile!.points,
      isNotEmpty,
  ...
  test(
    'selecting a profile with no curve reports an error and stays inactive',
    () async {
      final notifier = AutoEqNotifier(_StubDatabase(tmp));
      addTearDown(notifier.dispose);
      await notifier.downloadDatabase();
      ...
      final withoutCurve = notifier.state.searchResults.firstWhere(
        (p) => p.id == 8,
      );
